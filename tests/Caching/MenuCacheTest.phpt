...
class MenuCacheTest extends Tester\TestCase
{
    ...
    public function testSetArticleRepository()
    {
        ...
        Assert::type('App\Caching\MenuCache', $menuCache->setArticleRepository($this->articleRepository));
    }

    public function testSetImageRepository()
    {
        $menuCache = new MenuCache($this->netteCache, $this->tagRepository);
        Assert::type('App\Caching\MenuCache', $menuCache->setImageRepository($this->imageRepository));
    }

    public function testSetVideoRepository()
    {
        $menuCache = new MenuCache($this->netteCache, $this->tagRepository);
        Assert::type('App\Caching\MenuCache', $menuCache->setVideoRepository($this->videoRepository));
    }

    public function testSetWikiRepository()
    {
        $menuCache = new MenuCache($this->netteCache, $this->tagRepository);
        Assert::type('App\Caching\MenuCache', $menuCache->setWikiRepository($this->wikiRepository));
    }
